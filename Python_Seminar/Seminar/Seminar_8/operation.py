...
def record_data(dictt):
    ...
    with open('reference.csv', 'a', newline='', encoding='utf-8') as f:
        fieldnames = ['Имя', 'Фамилия', 'Отчество', 'Возраст', 'Должность', 'Телефон']
...
def reading_reference():
    try:
        with open('reference.csv', 'r', newline='', encoding='utf-8') as f:
            ...
            for row in reader:
                view_data('Имя: {}, Фамилия: {},  Отчество: {}, Возраст {}, '
                          'Должность {}, Телефон: {}'.format(row['Имя'],
                                                             row['Фамилия'],
                                                             row['Отчество'],
                                                             row['Возраст'],
                                                             row['Должность'],
        ...
        view_data('База работников отсутствует!\n')
